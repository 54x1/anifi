    ...
    <div class="min-h-screen bg-base-100 transition-colors duration-300">
        ...
        <div class="toast toast-top toast-end z-[70]" aria-live="polite" aria-atomic="true">
            <div v-for="t in toasts" :key="t.id" class="alert" :class="{
                ...
                <span>{{ t.message }}</span>
                <button class="btn btn-ghost btn-xs" @click="dismissToast(t.id)" aria-label="Dismiss notification">
                    ✕
                </button>
            </div>
        </div>
        ...
        <header class="navbar bg-base-200 shadow-lg sticky top-0 z-50" role="banner">
            ...
            <nav class="navbar-center hidden lg:flex" aria-label="Primary">
                <div class="tabs tabs-boxed">
                    <button v-for="tab in tabs" :key="tab.id" type="button" class="tab"
                        :class="{ 'tab-active': activeTab === tab.id }" @click="onTab(tab.id)"
            ...
            <div class="navbar-end">
                <div class="flex items-center gap-2">
                    <div class="stats shadow" aria-label="Available balance">
                        <div class="stat">
                    ...
                    <details class="dropdown dropdown-end">
                        <summary class="btn btn-ghost btn-circle" aria-haspopup="menu" aria-expanded="false"
                            title="Theme">
                            🎨 <span class="sr-only">Choose theme</span>
                        </summary>
                        <ul class="dropdown-content z-[60] menu p-2 shadow bg-base-100 rounded-box w-52 max-h-96 overflow-y-auto"
                            role="menu">
                            <li v-for="theme in availableThemes" :key="theme" role="none">
                                <button role="menuitem" class="justify-between"
                                    :class="{ active: currentTheme === theme }" @click="setTheme(theme)">
                                    {{ theme }}
                                    <span v-if="currentTheme === theme" aria-hidden="true">✓</span>
                                </button>
                            </li>
                        </ul>
                    </details>
          ...
          <li class="step" :class="{ 'step-primary': onboardingStep >= 1 }">Add data</li>
                            ...
                            <div v-if="onboardingStep === 0" class="text-center space-y-5">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <button class="btn btn-primary" @click="startImportFromOnboarding"
            ...
            <template v-else>
                ...
                <section v-if="showTips && transactions.length > 0 && activeTab === 'add'" class="alert alert-info mb-4"
                    ...
                    <section v-show="activeTab === 'transactions'" class="card bg-base-100 shadow-xl"
                        ...
                        <div class="card-body">
                            ...
                            <div v-if="showAdvancedTransactionsView" class="flex flex-wrap items-center gap-2 mb-4">
                                    ...
                                    <div class="dropdown-content card card-compact p-2 bg-base-100 shadow w-72">
                                        <div class="card-body space-y-2">
                                            <div class="form-control">
                                                <label class="label" for="thresh"><span class="label-text">Threshold
                                                        ($)</span></label>
                                                <input id="thresh" v-model.number="largeSelect.threshold" type="number"
                                                    min="1" class="input input-bordered input-sm" placeholder="50" />
                                            </div>
                                            <div class="form-control">
                                                <label class="cursor-pointer label" for="smartIqr">
                                                    <span class="label-text">Use smart threshold (IQR on
                                                        spending)</span>
                                            ...
                                            <div class="form-control">
                                                <label class="cursor-pointer label" for="pageOnly">
                                                    <span class="label-text">Scope is current page (else all
                                                        filtered)</span>
                                ...
                                <button class="btn btn-outline btn-xs" @click="openSmartSelect = true">
                                    🎯 Smart Select
                                </button>
                            ...
                            <div class="mb-4">
                                ...
                                <input id="txSearch" v-model="searchQuery" type="text"
                            ...
                            " class="flex flex-wrap justify-between items-center mb-3">
                                <label class="label cursor-pointer gap-2 text-sm" for="prioritizeSel">
                                    <input id="prioritizeSel" type="checkbox" class="checkbox checkbox-xs"
                                        v-model="prioritizeSelected" />
                                    <span>Prioritize selected on top</span>
                                </label>

                                <div class="flex flex-wrap gap-2 items-center">
                                    <button class="btn btn-accent btn-xs" :disabled="selectedCount === 0"
                                        @click="openBulkEdit">
                            ...
                            <div class="overflow-x-auto">
                                ...
                                <div class="grid gap-3 md:hidden">
                                    <article v-for="t in paginatedTransactions" :key="t.id"
                                        class="p-3 rounded-xl bg-base-200" :id="`tx-${t.id}`">
                                        <div class="flex items-start justify-between">
                                            ...
                                            <div class="text-right font-bold" :class="t.type === 'income' ? 'text-success' : 'text-error'
                                                ">
                                                ${{ t.amount.toFixed(2) }}
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-2 flex flex-wrap gap-1">
                                            <span v-for="tag in t.tags" :key="t.id + '-tag-' + tag"
                                                class="badge sm:badge-ghost">
                                                {{ tag }}
                                            </span>
                                            <span v-if="!t.tags?.length" class="text-xs opacity-50">No tags</span>
                                        </div>

                                        <div class="mt-2 flex flex-wrap gap-2 justify-end">
                                            <button class="btn btn-ghost btn-xs" @click="editTransaction(t)"
                                                aria-label="Edit">
                                                ✏️
                                            </button>
                                            <button class="btn btn-ghost btn-xs" @click="duplicateTx(t)"
                                                aria-label="Duplicate">
                                                📄
                                            </button>
                                            <button class="btn btn-ghost btn-xs text-error"
                                                @click="deleteTransaction(t.id)" aria-label="Delete">
                                                🗑️
                                            </button>
                                        </div>
                                    </article>
                                                ...
                                                <th scope="col">
                                                    <label>
                                                        <input type="checkbox" class="checkbox checkbox-sm"
                                                            :checked="allSelected" :indeterminate="someSelectedOnPage && !allSelected
                                            ...
                                            <tr v-for="(t, i) in paginatedTransactions" :key="t.id" :id="`tx-${t.id}`"
                                                ...
                                                <td>
                                                    <div class="flex flex-wrap gap-1">
                                                        <span v-for="tag in t.tags" :key="t.id + '-tg-' + tag"
                                                            class="badge sm:badge-ghost">{{ tag }}</span>
                                                        <span v-if="!t.tags?.length"
                            ...
                            <!-- Pagination -->
                            <div v-if="totalPages > 1" class="flex justify-center mt-6" role="navigation"
                                ...
                                <div class="btn-group">
                                    ...
                                    <button class="btn btn-sm" :disabled="currentPage === totalPages"
                                        @click="currentPage++" aria-label="Next page">
                                        ›
                                    </button>
                                    <button class="btn btn-sm" :disabled="currentPage === totalPages"
                ...
                <section v-show="activeTab === 'add'" class="card bg-base-100 shadow-xl mb-6"
                    ...
                    <div class="card-body">
                        ...
                        <form @submit.prevent="addTransaction"
                            ...
                            <!-- Date -->
                            <div class="form-control">
                                <label class="label" for="addDate"><span class="label-text">Date</span></label>
                                <div class="join w-full">
                                    <input id="addDate" ref="addDateTextRef" type="text" inputmode="numeric"
                                        placeholder="dd-mm-yyyy" v-model="newTxDateText" @input="onAddDateInput"
                                        @blur="onAddDateBlur" @keydown="onDateKeydownDigitsOnly"
                                        :aria-invalid="!!newTxDateError" aria-describedby="addDateError"
                                        class="input input-bordered join-item w-full" />
                                    <button type="button" class="btn btn-outline join-item" @click="openAddCalendar"
                                        title="Open calendar" aria-label="Open calendar">
                                        📅
                                    </button>
                                    <button type="button" class="btn btn-primary join-item" @click="setAddDateToday">
                                        Today
                                    </button>
                                </div>
                                ...
                                <input ref="addDatePickerRef" v-model="newTxDateISO" type="date" class="sr-only"
                                    tabindex="-1" aria-hidden="true" />
                            </div>

                            <!-- Type -->
                            <div class="form-control">
                                <label class="label" for="addType"><span class="label-text">Type</span></label>
                                <select id="addType" v-model="newTransaction.type" class="select select-bordered"
                                    required>
                            ...
                            <!-- Amount -->
                            <div class="form-control">
                                <label class="label" for="addAmount"><span class="label-text">Amount</span></label>
                                <div class="join">
                                    <span class="join-item btn btn-disabled" aria-hidden="true">$</span>
                                    <input id="addAmount" v-model.number="newTransaction.amount" type="number"
                            ...
                            <div class="relative">
                                <label class="label" :for="ids.input">
                                    ...
                                    <div class="ml-auto flex items-center gap-2">
                                        ...
                                        <button type="button" class="btn btn-ghost btn-xs"
                                            @click="openManager('manage')" aria-haspopup="dialog"
                                        ...
                                        <button type="button" class="btn btn-outline btn-xs"
                                            @click="openManager('manage')" title="Manage custom categories">
                                ...
                                <input :id="ids.input" v-model="query" type="text"
                                    class="input input-bordered w-full pr-20" :placeholder="placeholder" role="combobox"
                                    :aria-expanded="String(open)" :aria-controls="ids.listbox"
                                    :aria-activedescendant="open ? activeId : undefined" aria-autocomplete="list"
                                    autocomplete="off" @focus="open = true" @input="handleInput"
                                    @keydown.down.prevent="moveActive(1)" @keydown.up.prevent="moveActive(-1)"
                                    @keydown.enter.prevent="handleEnter" @keydown.esc.prevent="closeDropdown" />

                                <!-- Clear button -->
                                <button v-if="query" type="button"
                                    class="btn btn-ghost btn-xs absolute right-14 top-18 -translate-y-1/2" @click="
                                        query = '';
                                    open = true;
                                ...
                                <ul v-show="open"
                                    ...
                                    <li v-for="(c, i) in filteredAllCategories" :key="c">
                                        <button type="button" role="option" :id="optionId(i)"
                                            :aria-selected="activeIndex === i" class="justify-between"
                                    ...
                                    <!-- Empty state -->
                                    <li v-if="!showCreateOption && !filteredAllCategories.length"
                                ...
                                <div v-if="recentCategories.length && !trimmedQuery" class="mt-2 flex flex-wrap gap-1">
                                    <button v-for="rc in recentCategories" :key="rc" type="button"
                                        class="badge sm:badge-ghost hover:badge-primary" @click="selectCategory(rc)">
                                        {{ rc }}
                                    </button>
                                </div>
                            </div>
                            ...
                            <!-- Manager Modal -->
                            <dialog v-if="showManager" class="modal modal-open" aria-modal="true" role="dialog"
                                ...
                                <div class="modal-box w-full max-w-4xl max-h-[80vh] overflow-y-auto p-0">
                                    ...
                                    <div
                                        ...
                                        <button class="btn btn-circle btn-ghost btn-sm" @click="closeManager"
                                            aria-label="Close">
                                            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2">
                                    ...
                                    <div class="p-4">
                                        ...
                                        <div class="form-control mb-4">
                                            <label class="label">
                                                ...
                                                <span class="label-text-alt">{{ filteredCategories.length }}
                                                    categories</span>
                                            </label>
                                            <input v-model="categorySearch" type="text" placeholder="Search categories…"
                                                class="input input-bordered input-sm" data-testid="category-search" />
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                            ...
                                            <div>
                                                ...
                                                <div
                                                    ...
                                                    <button v-for="cat in filteredDefaults" :key="cat" type="button"
                                            ...
                                            <div>
                                                ...
                                                <div class="space-y-2 max-h-64 overflow-y-auto">
                                                    <div v-for="cat in filteredCustoms" :key="cat"
                                                        ...
                                                        <template v-if="editingCategory === cat">
                                                            <input ref="editInputRef" v-model="editingCategoryName"
                                                                type="text" class="input input-bordered input-xs flex-1"
                                                                @keyup.enter="saveEditCategory" @blur="saveEditCategory"
                                                        ...
                                                        <template v-else>
                                                            ...
                                                            <div class="flex flex-wrap gap-1">
                                                                ...
                                                                <button class="btn btn-ghost btn-xs text-error"
                                                                    @click="deleteCustomCategory(cat)" title="Delete">
                                                                    <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none"
                                                                        stroke="currentColor" stroke-width="2">
                                                                        <path
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            <input v-model="newCustomName" type="text"
                                                class="input input-bordered input-sm flex-1"
                                                placeholder="Enter new category name…" @keyup.enter="addCustomCategory"
                                                data-testid="new-category-input" />
                                            <button class="btn btn-primary btn-sm" @click="addCustomCategory">
                                                Add Category
                                            </button>
                                        </div>
                                        ...
                                        <div class="form-control mb-4">
                                            <label class="label">
                                                ...
                                                <span class="label-text-alt">({{ tags.length }} tags)</span>
                                            </label>
                                            ...
                                            <div class="flex flex-wrap gap-2 mb-2">
                                                <div v-for="tag in filteredTags" :key="tag"
                                                    ...
                                                    <button type="button" class="btn btn-ghost btn-xs text-error"
                                                        @click="removeTag(tag)" title="Remove tag">
                                            ...
                                            <div class="join w-full">
                                                ...
                                                <button class="btn btn-primary btn-sm join-item" @click="addNewTag">
                                                    Add Tag
                                                </button>
                                            </div>
                                            ...
                                            <div v-if="tagSuggestions.length" class="mt-2 text-sm">
                                                ...
                                                <button v-for="s in tagSuggestions" :key="s"
                                                    class="btn btn-ghost btn-xs" @click="quickAddTag(s)">
                                                    {{ s }}
                                                </button>
                                            </div>
                                        </div>
      ...
      <button class="btn" @click="closeManager">Close</button>
                            ...
                            <div class="form-control">
                                <label class="label">
                                    <span class="label-text">Tags</span>
                                    <span class="label-text-alt">Type and press Enter (or comma)</span>
                                </label>

                                <div class="join w-full">
                                    <input v-model="tagSearch" type="text" placeholder="Search or add new tag…"
                                        class="input input-bordered input-sm join-item w-full"
                                        @keydown.enter.prevent="addNewTag" />
                                    <button class="btn btn-primary btn-sm join-item" @click="addNewTag">
                                        Add Tag
                                ...
                                <!-- Current tags -->
                                <div class="flex flex-wrap gap-2 mb-2">
                                    <span v-for="tag in newTransaction.tags" :key="tag"
                                        class="badge badge-primary gap-1">
                                        <span>{{ tag }}</span>
                                        <button type="button" class="btn btn-ghost btn-xs text-error"
                                            @click="removeTagFromTransaction(tag)" aria-label="Remove tag">
                                    ...
                                    <span v-if="!newTransaction.tags.length" class="text-xs opacity-60">No tags
                                        yet</span>
                                </div>
                            ...
                            <!-- Description -->
                            <div class="form-control">
                                <label class="label" for="addDesc"><span class="label-text">Description
                                        (Optional)</span></label>
                            ...
                            <div class="form-control">
                                <label class="label cursor-pointer" for="addRecurring">
                                    ...
                                    <input id="addRecurring" v-model="newTransaction.recurring" type="checkbox"
                                        class="checkbox" />
                                </label>
                                <div v-if="newTransaction.recurring" class="space-y-2">
                                    ...
                                    <select id="addFreq" v-model="newTransaction.frequency"
                                        class="select select-bordered select-sm" aria-describedby="recurringHelp">
                                    ...
                                    <label class="sr-only" for="addRecursions">Number of occurrences</label>
                                    <input id="addRecursions" v-model.number="newTransaction.recursions" type="number"
                                        min="1" max="365" placeholder="Number of occurrences"
                                        class="input input-bordered input-sm" aria-describedby="recurringHelp" />
                ...
                <section v-show="activeTab === 'import'" class="card bg-base-100 shadow-xl mb-6"
                    ...
                    <div class="card-body">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-6">
                                <!-- File upload -->
                                <div class="form-control">
                                    <label class="label" for="csvUpload"><span class="label-text">Upload CSV
                                            File</span></label>
                                    <input id="csvUpload" type="file" accept=".csv" multiple
                                        class="file-input file-input-bordered w-full" @change="handleFileUpload"
                                        ref="csvInputRef" />
                                ...
                                <!-- URL / Share code -->
                                <div class="form-control">
                                    <label class="label" for="importUrl"><span class="label-text">Import from URL or
                                            Share
                                            Code</span></label>
                                    <div class="join">
                                        <input id="importUrl" v-model="importUrl" type="text" inputmode="url"
                                            autocapitalize="off" autocorrect="off"
                                            placeholder="https://... or share code (tx:...)"
                                            class="input input-bordered join-item flex-1" />
                                        <button class="btn btn-primary join-item" @click="importFromUrlOrCode">
                                            Import
                                        </button>
                                    </div>
                                    <p class="text-xs text-base-content/60 mt-1">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <button class="btn btn-error btn-sm" @click="clearAllTransactions">
                                ...
                                <div v-if="scanning" class="card bg-base-200 shadow-inner p-3">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button class="btn btn-ghost btn-sm" @click="stopQrScan"
                                            aria-label="Stop scanning">
                                            ✕
                                        </button>
                                    </div>
                                    <video ref="qrVideo" class="w-full rounded-lg mt-2" autoplay playsinline
                                        muted></video>
                            ...
                            <div class="space-y-6">
                                <!-- Share code -->
                                <div class="form-control">
                                    <label class="label" for="shareCode"><span class="label-text">Share Code (compact,
                                            paste
                                            anywhere)</span>
                                        <p class="text-xs text-base-content/60 mt-1">
                                            Length:
                    ...
                    <div class="text-xs text-error mt-1" v-if="qrError">
                      {{ qrError }}
                                ...
                                <!-- JSON Export / Import -->
                                <div class="form-control">
                                    <label class="label"><span class="label-text">JSON Export / Import</span></label>
                                    <div class="flex flex-wrap gap-2 items-center">
                    ...
                    <div class="card bg-base-100 shadow-xl">
                        <div class="card-body">
                            ...
                            <div class="flex flex-wrap gap-2 mb-4">
                                <button v-for="preset in datePresets" :key="preset.label" class="btn btn-sm" :class="{
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="form-control">
                                    <label class="label" for="startDate"><span class="label-text">Start
                                            Date</span></label>
                                    <input id="startDate" v-model="dateFilter.start" type="date"
                                        class="input input-bordered" />
                                </div>
                                <div class="form-control">
                                    <label class="label" for="endDate"><span class="label-text">End Date</span></label>
                                    <input id="endDate" v-model="dateFilter.end" type="date"
                                        class="input input-bordered" />
                                </div>
                            </div>
                    ...
                    <div class="card bg-base-100 shadow-xl">
                        <div class="card-body">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div class="form-control">
                                    <label class="label" for="chartType"><span class="label-text">Chart
                                            Type</span></label>
                                ...
                                <div class="form-control">
                                    ...
                                    <div class="flex flex-wrap gap-1 max-h-32 overflow-y-auto" role="listbox"
                                        aria-label="Selected categories">
                                        <button v-for="category in chartCategories" :key="category" type="button"
                                            class="badge badge-outline cursor-pointer text-xs" :class="{
                                                'badge-primary': selectedCategories.includes(category),
                                            }" @click="toggleCategory(category)"
                                            :aria-pressed="selectedCategories.includes(category)">
                                            {{ category
                                            }}<span v-if="selectedCategories.includes(category)" class="ml-1"
                                                aria-hidden="true">✕</span>
                                        </button>
                                    </div>
                                    <div class="mt-2">
                                        <button class="btn btn-xs btn-ghost" @click="selectAllCategories">
                                            Select All
                                        </button>
                                        <button class="btn btn-xs btn-ghost" @click="unselectAllCategories">
                                            Unselect All
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="form-control md:col-span-3">
                                    ...
                                    <div class="flex flex-wrap items-center gap-4 text-sm">
                                        ...
                                        <label class="inline-flex items-center gap-2">
                                            ...
                                            <span>Avaliable Balance (Period)</span>
                                        </label>

                                        <label class="inline-flex items-center gap-2">
                                            <input type="checkbox" class="checkbox checkbox-xs"
                                                v-model="seriesToggles.allTimeCumulativeNetBalance" />
                            ...
                            <div class="stats stats-vertical lg:stats-horizontal shadow mt-6 w-full">
                                ...
                                <div class="stat">
                                    <div class="stat-title">Avaliable Balance</div>
                ...
                <div class="card-body">
                    ...
                    <div class="prose max-w-none space-y-4">
                        ...
                        <div v-if="!transactions.length" class="text-center mt-8">
                            <button class="btn btn-primary btn-wide btn-lg" @click="startTourFromAbout">
                                Get Started Now
                            </button>
                        </div>
...
<li>Remember last selected option</li>
        ...
        <dialog v-if="catMgr.open" class="modal modal-open" aria-modal="true" role="dialog"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
                    ...
                    <div>
                        ...
                        <ul class="bg-base-200 rounded-box p-2 max-h-56 overflow-auto divide-y divide-base-300">
                            <li v-for="c in customCategories" :key="c" class="flex items-center gap-2 py-1">
                                <template v-if="catMgr.renaming === c">
                                    <input v-model="catMgr.renameText" type="text"
                                        class="input input-bordered input-xs flex-1" @keyup.enter="saveRenameCategory()"
                                        @blur="saveRenameCategory()" />
                                    <button class="btn btn-ghost btn-xs" @click="cancelRenameCategory"
                                        aria-label="Cancel">
                                        <!-- x icon -->
                                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                            stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <template v-else>
                                    ...
                                    <button class="btn btn-ghost btn-xs" @click="startRenameCategory(c)"
                                        aria-label="Rename">
                                        <!-- pencil -->
                                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                            stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"
                                    ...
                                    <button class="btn btn-ghost btn-xs text-error" @click="deleteCustomCategory(c)"
                                        aria-label="Delete">
                                        <!-- trash -->
                                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                            stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <div class="join mt-2 w-full">
                            <input v-model="catMgr.newName" type="text"
                                class="input input-bordered input-sm join-item flex-1" placeholder="New custom category"
                                @keyup.enter="addCustomCategory()" />
                            <button class="btn btn-primary btn-sm join-item" @click="addCustomCategory"
                                aria-label="Add">
                                <!-- plus -->
                                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            ...
            <div class="modal-box w-full max-w-2xl p-0 md:max-h-[80vh]">
                ...
                <div class="p-3 bg-base-200 sticky top-0 z-10">
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <button class="btn btn-ghost btn-sm" @click="closeTagPicker" aria-label="Close">
                            ✕
                        </button>
                    </div>
                    <div class="mt-2 grid grid-cols-[1fr_auto] gap-2">
                        <input ref="tagPickerInputRef" v-model="tagPicker.q" type="text"
                            class="input input-bordered input-sm w-full" placeholder="Search or create…"
                            autocomplete="off" @keydown.enter.prevent="createTagFromQuery()" />
                        <button class="btn btn-primary btn-sm" @click="createTagFromQuery()">
                            Add
                        </button>
                    </div>
        ...
        <dialog v-if="labelImport.open" class="modal modal-open" aria-modal="true" role="dialog"
            ...
            <div class="modal-box w-full max-w-md">
                ...
                <p class="mt-2 text-sm text-base-content/70">
                    ...
                    <span v-if="labelImport.filename">(<strong>{{ labelImport.filename }}</strong>)</span>
                </p>
                ...
                <div class="modal-action">
                    ...
                    <button class="btn btn-primary" :disabled="!labelImport.label.trim()" @click="confirmLabelImport">
                        Import
                    </button>
                </div>
            </div>
        ...
        <dialog v-if="openSmartSelect" class="modal modal-open" aria-modal="true" role="dialog"
            ...
            <div class="modal-box w-full max-w-lg">
                ...
                <div class="py-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div class="form-control">
                        <label class="label" for="ssCategory"><span class="label-text">Category</span></label>
                    ...
                    <div class="form-control">
                        <label class="label" for="ssType"><span class="label-text">Type</span></label>
                    ...
                    <div class="form-control">
                        <label class="label" for="ssMin"><span class="label-text">Min Amount</span></label>
                        <input id="ssMin" v-model.number="smartSelect.min" type="number" step="0.01"
                            class="input input-bordered input-sm" />
                    </div>

                    <div class="form-control">
                        <label class="label" for="ssMax"><span class="label-text">Max Amount</span></label>
                        <input id="ssMax" v-model.number="smartSelect.max" type="number" step="0.01"
                            class="input input-bordered input-sm" />
                    </div>

                    <div class="form-control">
                        <label class="label" for="ssFrom"><span class="label-text">From</span></label>
                        <input id="ssFrom" v-model="smartSelect.from" type="date"
                            class="input input-bordered input-sm" />
                    </div>

                    <div class="form-control">
                        <label class="label" for="ssTo"><span class="label-text">To</span></label>
                        <input id="ssTo" v-model="smartSelect.to" type="date" class="input input-bordered input-sm" />
                    </div>

                    <div class="form-control md:col-span-2">
                        <label class="label" for="ssContains"><span class="label-text">Description
                                contains</span></label>
                        <input id="ssContains" v-model="smartSelect.contains" type="text"
                            class="input input-bordered input-sm" />
                    </div>
                </div>
        ...
        <dialog v-if="bulkEdit.open" class="modal modal-open" aria-modal="true" role="dialog"
            ...
            <div class="modal-box w-full max-w-lg">
                ...
                <div class="mt-3 space-y-3">
                    ...
                    <div class="form-control">
                        ...
                        <div class="flex flex-wrap gap-3 items-center">
                            <label class="inline-flex gap-2 items-center">
                                <input type="radio" name="beType" value="" v-model="bulkEdit.type"
                                    class="radio radio-xs" />
                                <span>(no change)</span>
                            </label>
                            <label class="inline-flex gap-2 items-center">
                                <input type="radio" name="beType" value="income" v-model="bulkEdit.type"
                                    class="radio radio-xs" />
                                <span>Income</span>
                            </label>
                            <label class="inline-flex gap-2 items-center">
                                <input type="radio" name="beType" value="spending" v-model="bulkEdit.type"
                                    class="radio radio-xs" />
                                <span>Spending</span>
                            </label>
                        </div>
                    ...
                    <!-- Smart: Find & Replace -->
                    <div class="form-control">
                        <label class="label" for="beFind"><span class="label-text">Find & Replace
...
function parseCSV(csvText: string, filename?: string) {
    ...
    lastImportSummary.value =
        `Parsed ${imported.length} transaction${imported.length === 1 ? "" : "s"
...
function confirmLabelImport() {
    ...
    lastImportSummary.value = `Imported ${count} transaction${count === 1 ? "" : "s"
        } from "${label}"`;
...
function loadTransactionsFromJSON(json: string) {
    ...
    lastImportSummary.value = `Imported ${imported} transaction${imported === 1 ? "" : "s"
        } from shared data.`;
    setImportStatus(lastImportSummary.value);
}
...
const seriesToggles = ref<{
    ...
    spending: boolean;
    balance: boolean;
    allTimeCumulativeNetBalance: boolean;
}>({
    income: true,
    spending: true,
    balance: true,
...
const chartData = computed(() => {
    ...
    // Visible period buckets (respects current date filters)
    ...
    // All-time cumulative running net balance
    // Uses all transactions, not just the currently filtered date window,
    // but still respects selected category filters for chart consistency.
    const allTimeBase =
        selectedCategories.value.length > 0
